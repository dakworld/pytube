...
class SearchView(generic.ListView):
    ...
    def get_queryset(self):
        ...
        Entry.objects.annotate(search=SearchVector('title', 'uploader', 'description', 'comment__message'),).annotate(similarity=TrigramSimilarity('search', query),).filter(similarity__gt=0.2).order_by('-similarity')
        return filtered
